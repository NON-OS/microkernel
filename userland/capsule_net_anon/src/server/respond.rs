...
//! Writing a reply back to the caller.

use nonos_libc::mk_ipc_reply;

use crate::protocol::{HDR_LEN, MAGIC, VERSION};

/// Reply to `pid`. The body must already be in `tx` past the header.
pub fn respond(pid: u32, op: u16, errno: u16, request_id: u32, payload_len: u32, tx: &mut [u8]) {
    ...
    let _ = mk_ipc_reply(pid, tx.as_ptr(), HDR_LEN + payload_len as usize);
}
