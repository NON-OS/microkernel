...
//! The sRGB channel values in linear light, as WCAG 2.1 defines them.

/*
 * Linearised sRGB: c/12.92 below 0.03928, and ((c+0.055)/1.055).powf(2.4) above,
 * which is the transfer function WCAG 2.1 gives for relative luminance. Scaled to
 * 16 bits so a luminance sum stays exact in integer arithmetic.
 *
 * A table rather than the power function itself. There are 256 possible channel
 * values, so nothing is approximated by tabulating them: this is the function, at
 * every input it can be asked about, and it costs one load instead of a pow.
 * `the_table_is_the_wcag_transfer_function` recomputes all 256 in floating point
 * and fails if any entry drifts.
 */
pub const LINEAR: [u16; 256] = [
