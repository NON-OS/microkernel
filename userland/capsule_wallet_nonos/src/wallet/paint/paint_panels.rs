...
fn lock_screen(fb: &mut PaintBuffer) {
    ...
    center(fb, cx, 352, "Wallet locked", FG(), scale::TITLE);
    center(fb, cx, 386, "Click anywhere to unlock", MUTED(), scale::BODY);
}

fn command(fb: &mut PaintBuffer) {
    let x = 410u32;
    ui::card(fb, x, 150, 460, 396);
    let _ = fb.text_ttf((x + 20) as i32, 166, "COMMAND PALETTE", DIM(), scale::BODY);
    ui::bordered(fb, x + 20, 188, 420, 34, PANEL_2(), LINE());
    let _ = fb.text_ttf((x + 32) as i32, 197, "Type a command\u{2026}", MUTED(), scale::BODY);
    for (i, cmd) in CMDS.iter().enumerate() {
        let y = 240 + i as u32 * 36;
        let _ = fb.text_ttf((x + 24) as i32, (y + 8) as i32, cmd, FG(), scale::BODY);
    }
}
...
fn messages(fb: &mut PaintBuffer) {
    ...
    let _ = fb.text_ttf((x + 18) as i32, 98, "NOTIFICATIONS", DIM(), scale::BODY);
    // No event feed is wired, so the panel does not fabricate one.
    let _ = fb.text_ttf((x + 18) as i32, 132, "No notifications", MUTED(), scale::BODY);
}

fn account(fb: &mut PaintBuffer, state: &State) {
    let x = 994u32;
    ui::card(fb, x, 82, 260, 96);
    let _ = fb.text_ttf((x + 18) as i32, 98, "ACCOUNT", DIM(), scale::BODY);
    fb.fill_rect(x + 8, 118, 244, 34, SEL());
    fb.fill_rect(x + 8, 118, 3, 34, ACCENT());
    let _ = fb.text_ttf((x + 20) as i32, 122, "Main", FG(), scale::BODY);
    if state.address_ready {
        let mut sa = [0u8; 13];
        super::super::hex::short_addr(&state.address, &mut sa);
        let s = core::str::from_utf8(&sa).unwrap_or("");
        let _ = fb.text_ttf_mono((x + 20) as i32, 138, s, MUTED(), scale::BODY);
    } else {
        let _ = fb.text_ttf((x + 20) as i32, 138, "not generated", MUTED(), scale::BODY);
    }
}
