...
pub fn paint_send_side(state: &State, fb: &mut PaintBuffer) {
    ...
    let _ = fb.text_ttf((x + 20) as i32, 164, "ROUTE", DIM(), scale::BODY);
    let _ = fb.text_ttf((x + 20) as i32, 186, "PublicNode Ethereum RPC", FG(), scale::VALUE);
...
fn kv(fb: &mut PaintBuffer, x: u32, w: u32, y: u32, k: &str, v: &str) {
    let _ = fb.text_ttf((x + 20) as i32, y as i32, k, MUTED(), scale::BODY);
    let vw = fb.measure_ttf(v, scale::BODY).max(0) as u32;
    let _ = fb.text_ttf((x + w - 20 - vw) as i32, y as i32, v, FG(), scale::BODY);
}
