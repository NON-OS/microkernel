...
pub fn paint_sysbar(fb: &mut PaintBuffer) {
    ...
    let _ = fb.text_ttf_mono(34, 9, "CAPSULE_WALLET_NONOS  CPL=3", MUTED(), scale::BODY);
    let right = "12:39  fps 60  1280x800";
    let w = fb.measure_ttf_mono(right, scale::BODY).max(0) as u32;
    let _ = fb.text_ttf_mono((WIDTH - 16 - w) as i32, 9, right, DIM(), scale::BODY);
}
